function [ server_util, Q, P, status, count ] = getServerParamGP_Active( rt_tc, se_tc,...
    n_low_prio, rt_deadline_tol )
...
while 1
    ...
    for i=startIndex:rt_tc.ntask
        ...
        % Dj = rt_tc.periods(i);
        Dj_tol = rt_tc.deadlines(i) + rt_tc.deadlines(i)*rt_deadline_tol;
        Ij = getInterferenceWithDeadline( i, rt_tc.wcets, Dj_tol, rt_tc.periods );
        Ij = Ij/Dj_tol;
        
        constr = [constr; Ij  + (((Dj_tol + P ) * (P)^(-1))*Q)/Dj_tol  <= 1];
    end
